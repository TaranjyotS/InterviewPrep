...
4. The server returns an HTTP response with:  
   - A status code (e.g. `200 OK`, `201 Created`, `400 Bad Request`, `404 Not Found`).  
   - A response body (often JSON).  
   - Response headers (e.g. content type, cache headers).  
...
2. **HTTP Basic Auth**  
   ...
   - Suitable mostly for internal systems or via HTTPS only.

3. **Bearer Tokens / JWT**  
   - Short-lived tokens passed as `Authorization: Bearer <token>`.  
   - JWTs contain claims about the user and can be verified with cryptographic signatures.

4. **OAuth 2.0**  
   - Standard for delegated authorization.  
   - Used by many identity providers (Google, GitHub, etc.).

5. **AWS Signature Version 4**  
   - AWS-specific request signing mechanism used for calling AWS APIs securely.
...
- Follow **consistent naming conventions** (snake_case or camelCase) in JSON.  
...
1. **URL Versioning (most common)**  
   - `/v1/users`, `/v2/users`  
   - Simple and visible; easy to route.

2. **Header Versioning**  
   - `Accept: application/vnd.myapi.v1+json`  
   - Cleaner URLs but more complex routing.

3. **Query Parameter Versioning**  
   - `/users?version=1`  
   - Generally less preferred but sometimes used.
...
def verify_api_key(x_api_key: str = Header(..., alias=API_KEY_HEADER)):
